...
$element_type = $meta->getMetaStr($rs->post_meta,'map');
$element_type_options = $meta->getMetaStr($rs->post_meta,'map_options');


if ($element_type_options != '') {
	$map_options = explode(",",$element_type_options);
...
if ($element_type !='') {
	$maps_array = explode(",",$element_type);
...
<html>
	<head>
		...
		$starting_script .= dcPage::jsLoad(DC_ADMIN_URL.'?pf=myGmaps/js/add.map.js');
...
if (!$core->error->flag())
{
	...
	'<input type="hidden" name="myGmaps_type" value="'.$myGmaps_type.'" /></p>'.
	'</div>'.
	'</form>',
	$show_filters
	);
	...
	echo '<div class="multi-part" id="settings" title="'.__('Map parameters').'">'.
	'<p class="clear">'.__('Choose options for map attached to post:').' <a href="'.$core->getPostAdminURL($rs->post_type,$rs->post_id).'">'.$post_title.'</a>';
	...
	echo '</p>';
	echo
	'<form method="post" action="'.$p_url.'" id="map-options">'.
	'<div class="fieldset"><h3>'.__('Map parameters').'</h3>'.
