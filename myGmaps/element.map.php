...
<html>
	<head>
		...
		dcPage::jsLoad(DC_ADMIN_URL.'?pf=myGmaps/js/element.map.post.js').
	...
	echo '<script type="text/javascript" src="http://maps.google.com/maps/api/js?libraries=weather,places&amp;sensor=false"></script>
	// Add translations for element.map.js';
...
if ($can_edit_post)
{
	...
	$main_items = new ArrayObject(array(
		...
		"post_excerpt" =>
			'<p class="area" id="excerpt"><label for="post_excerpt" class="bold">'.__('Position:').' </label> '.
			'<span class="hidden">'.form::textarea('post_excerpt',50,5,html::escapeHTML($post_excerpt)).'</span></p>'.
			...
			'<button id="add_weather" class="add_weather" type="button" title="'.__('Weather').'"><span>'.__('Weather').'</span></button>'.
