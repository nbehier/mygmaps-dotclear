...
class mygmapsPublic
{
    const MAP_API = 'http://maps.google.com/maps/api/js';
...
EOT;
    ...
    public static function publicJsContent(array $aOptions) {
        return '<script type="text/javascript" src="' . self::MAP_API . '?libraries=weather&amp;sensor=false"></script>'."\n";
    }
...
EOT;
        ...
        elseif ($sType == 'weather') {
            $sOutput .= self::getMapElementWeatherOptions($aElementOptions);
        }
...
EOT;
    ...
    protected static function getMapElementWeatherOptions(array $aOptions) {
        self::checkOptions(
            get_called_class(), 
            array_keys($aOptions), 
            array('map_id')
...
weatherLayer.setMap(map_{$sMapId});\n
EOT;
        return $sOutput;
    }
